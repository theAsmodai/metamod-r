...
mBOOL MPlugin::ini_parseline(const char *line)
{
	...
	cp = tmp_line + Q_strlen(tmp_line) -1;
	while (*cp == ' ' || *cp == '\t') *cp-- = '\0';
	...
	if (tmp_line[0] == '#' || tmp_line[0] == ';' || Q_strstr(tmp_line, "//") == tmp_line)
	{
...
mBOOL MPlugin::check_input()
{
	...
	if (!file || !file[0])
	{
		META_WARNING("dll: Tried to operate on plugin[%d] with an empty file", index);
		RETURN_ERRNO(mFALSE, ME_ARGUMENT);
	}

	if (!filename[0])
	{
		META_WARNING("dll: Tried to operate on plugin[%d] with an empty filename", index);
		RETURN_ERRNO(mFALSE, ME_ARGUMENT);
	}

	if (!pathname[0])
	{
		META_WARNING("dll: Tried to operate on plugin[%d] with an empty pathname", index);
		RETURN_ERRNO(mFALSE, ME_ARGUMENT);
	}
...
mBOOL MPlugin::platform_match(MPlugin *other)
{
	...
	if (!Q_strcmp(file, other->file))
		return mTRUE;

	if (status >= PL_OPENED && other->status >= PL_OPENED && !Q_strcmp(info->logtag, other->info->logtag))
		return mTRUE;

	if (*desc != '\0' && !Q_stricmp(desc,other->desc))
		return mTRUE;
	...
	if (!Q_strncmp(file,other->file, prefixlen))
		return mTRUE;
...
mBOOL MPlugin::attach(PLUG_LOADTIME now)
{
	...
	if (!gamedll_funcs.newapi_table)
	{
		...
		if (GameDLL.funcs.newapi_table)
			Q_memcpy(gamedll_funcs.newapi_table, GameDLL.funcs.newapi_table, sizeof(NEW_DLL_FUNCTIONS));
		else
...
#define GET_FUNC_TABLE_FROM_PLUGIN(pfnGetFuncs, STR_GetFuncs, struct_field, API_TYPE, TABLE_TYPE, TABLE_SIZE, vers_pass, vers_int, vers_want)\
	if (meta_table.pfnGetFuncs) {\
		if (!struct_field) {\
			...
			Q_memset(struct_field, 0, TABLE_SIZE);\
		}\
...
mBOOL MPlugin::retry(PLUG_LOADTIME now, PL_UNLOAD_REASON reason)
{
	switch (action)
	{
	case PA_LOAD:	return load(now);
	case PA_ATTACH:	return load(now);
	case PA_UNLOAD:	return unload(now, reason, reason);
	case PA_RELOAD:	return reload(now, reason);
	default:
	{
...
const char *MPlugin::str_loadtime(PLUG_LOADTIME ptime, STR_LOADTIME fmt)
{
	...
	if (fmt == SL_SHOW)
		return META_UTIL_VarArgs("UNK-%d", ptime);
