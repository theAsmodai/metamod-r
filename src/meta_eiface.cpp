	...
	)
{
	...
	Q_memset( dummies, 0, sizeof(pdummyfunc) * c_NumDummies ); 
}
    ...
    )
{
	...
	Q_memset( extra_functions, 0, sizeof(extra_functions));
	Q_memset( dummies, 0, sizeof(pdummyfunc) * c_NumDummies ); 
}
...
// 158: int		(*pfnCheckParm)			( const char *pchCmdLineToke, char **pchNextValue );
