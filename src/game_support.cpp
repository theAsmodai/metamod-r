...
mBOOL DLLINTERNAL setup_gamedll(gamedll_t *gamedll) {
	...
	if((known=lookup_game(gamedll->name))) {
		...
		if((cp = Q_strstr(knownfn, "_i386.so")) ||
		   (cp = Q_strstr(knownfn, "_i486.so")) ||
		   (cp = Q_strstr(knownfn, "_i586.so")) ||
		   (cp = Q_strstr(knownfn, "_i686.so"))) {
		   	//make sure that it's the ending that has "_iX86.so"
		   	if(cp[Q_strlen("_i386.so")] == 0) {
...
#error "OS unrecognized"
		...
		if(!Config->gamedll) {
			...
			char temp_str[NAME_MAX], *strippedfn;

			STRNCPY(temp_str, knownfn, sizeof(temp_str));
			strippedfn = temp_str;

			char *loc = Q_strrchr(strippedfn, '_');
